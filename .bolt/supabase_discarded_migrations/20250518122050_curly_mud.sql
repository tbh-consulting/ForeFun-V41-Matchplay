...
BEGIN
  IF NOT EXISTS (
    ...
    -- First add as text column with default value
    ALTER TABLE scorecards
    ADD COLUMN game_type text DEFAULT 'stroke';
    
    -- Then convert to enum type
    ALTER TABLE scorecards 
    ALTER COLUMN game_type TYPE game_type 
    USING 
      CASE 
        WHEN game_type = 'stroke' THEN 'strokeplay'::game_type
        ELSE 'strokeplay'::game_type
      END;
    
    -- Set the default value to the enum type
    ALTER TABLE scorecards
    ALTER COLUMN game_type SET DEFAULT 'strokeplay';
  END IF;
END$$;
...
-- Drop existing create_scorecard functions to avoid conflicts
