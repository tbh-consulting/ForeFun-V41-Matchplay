/*
  ...
  1. Changes
    - Fixes the `calculate_team_match_play_total()` function to correctly sum all match play statuses for a team
    - Removes the scorecard_id filter to ensure all match play statuses are included in the total
    - Adds a function to handle team score deletion and recalculate match play totals
    - Adds a utility function to recalculate all match play totals for existing data
...
BEGIN
  ...
  -- Remove the scorecard_id filter to sum all match play statuses
...
-- Create a function to handle team score deletion
...
BEGIN
  ...
  -- Recalculate match play total for the deleted team's score
  SELECT COALESCE(SUM(match_play_status), 0)
  INTO total_match_play
  FROM scorecard_team_scores
  ...
  -- If we found another team, recalculate its match play total too
  IF other_team_id IS NOT NULL THEN
    ...
    -- Update the other team's match_play_total
    UPDATE teams
    SET match_play_total = total_match_play
...
-- Create trigger for handling team score deletion
...
-- Recalculate all match play totals for existing data
