...
-- Add team_id column to scorecard_players if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
...
-- Add game_type column to scorecards if it doesn't exist
DO $$
BEGIN
  -- First check if the enum type exists
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'game_type') THEN
    CREATE TYPE game_type AS ENUM ('strokeplay', 'scramble', '4ball');
  END IF;
...
-- Update create_scorecard function signature to avoid conflicts
CREATE OR REPLACE FUNCTION create_scorecard_with_game_type(
  p_course_id uuid,
  p_created_by uuid,
  p_date timestamptz,
...
GRANT EXECUTE ON FUNCTION create_scorecard_with_game_type TO authenticated;