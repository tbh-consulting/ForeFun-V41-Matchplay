...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_teams_total_gross_score'
  ) THEN
    CREATE INDEX idx_teams_total_gross_score ON teams(total_gross_score);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_teams_total_points'
  ) THEN
    CREATE INDEX idx_teams_total_points ON teams(total_points);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_teams_relative_score'
  ) THEN
    CREATE INDEX idx_teams_relative_score ON teams(relative_score);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_indexes WHERE indexname = 'idx_teams_completed_holes'
  ) THEN
    CREATE INDEX idx_teams_completed_holes ON teams(completed_holes);
  END IF;
END$$;

-- Drop existing functions if they exist to avoid conflicts
DROP FUNCTION IF EXISTS initialize_team_scores(uuid, uuid, numeric) CASCADE;
DROP FUNCTION IF EXISTS update_team_score_points() CASCADE;
DROP FUNCTION IF EXISTS update_team_total_gross_score() CASCADE;
DROP FUNCTION IF EXISTS update_team_total_points() CASCADE;
DROP FUNCTION IF EXISTS update_team_completed_holes() CASCADE;
DROP FUNCTION IF EXISTS calculate_team_relative_score() CASCADE;

-- Function to calculate team handicap strokes for a hole
...
-- Create new triggers with DO blocks to check if they exist first
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_team_points') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_team_total_gross_score') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_team_total_points') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_team_completed_holes') THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_team_relative_score') THEN
