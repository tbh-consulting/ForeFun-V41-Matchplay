/*
  # Fix Team Scorecard Creation

  1. Changes
    - Fix the create_team_scorecard function to properly initialize team scores
    - Fix the player loop in create_team_scorecard to correctly add players to teams
    - Ensure handicap strokes are calculated correctly for team scores
    - Add proper error handling and logging

  2. Impact
    - Team scorecards will now properly create scorecard_team_scores records
    - Points will be calculated correctly for team games
    - Handicap strokes will be applied correctly
*/

-- Drop existing function to avoid conflicts
DROP FUNCTION IF EXISTS create_team_scorecard(uuid, uuid, timestamptz, weather_condition, game_type, jsonb);

-- Create improved function to handle team scorecard creation
