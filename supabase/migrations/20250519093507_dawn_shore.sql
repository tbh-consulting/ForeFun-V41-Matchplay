/*
  ...
  1. Changes
     - Updates the match play total calculation to recalculate for ALL teams in a scorecard
     - Ensures when one team's score changes, all teams in that scorecard have their totals updated
     - Fixes the issue where only the changed team's total was being updated
  
  2. Impact
     - Teams.match_play_total will now correctly reflect the sum of all match_play_status values
     - Match play scoring will work correctly for team games
