-- Improve the match play status calculation function to correctly handle handicap strokes
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' OR TG_OP = 'INSERT' THEN
    ...
    SELECT team_id, gross_score, handicap_strokes, hole_si
    INTO other_team_id, other_team_score, other_team_handicap_strokes, hole_si
    ...
    IF other_team_id IS NOT NULL AND NEW.gross_score IS NOT NULL AND other_team_score IS NOT NULL THEN
      ...
      team1_strokes := CASE WHEN team1_handicap >= hole_si THEN 1 ELSE 0 END;
      team2_strokes := CASE WHEN team2_handicap >= hole_si THEN 1 ELSE 0 END;
